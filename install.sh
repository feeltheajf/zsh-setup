...
sudo -K
sh -c "sudo ${package_manager} zsh &> /dev/null" 
...
sh -c "cd ${prefix}/bin/fasd && sudo make install &> /dev/null"
sh -c "mkdir -p ~/.config/peco && cp ${prefix}/user/config.json ~/.config/peco/ && peco --rcfile ~/.config/peco/config.json &> /dev/null"
